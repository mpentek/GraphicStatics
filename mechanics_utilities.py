...
from plot_utilities import plot_fr_poly, plot_fun_poly,plot_method_of_joints,plot_poleline,plot_cremona, plot_cutting_line
...
def funicular_polygon(ax_fr_poly,ax_fun_poly,forces,system):
    ...
    intersection_point = system.ex_forces_list[0][0].line.intersection(baseline)

    intersections = []
    if intersection_point is not None:
        intersections.append(intersection_point)
    else:
        intersections.append(system.ex_forces_list[0][0].p1)
    ...
    for i in range(len(forces)-1):
        ...
        intersections.append(polelines[-1].intersection(forces[i+1][0].line))
    ...
    intersections.append(polelines[-1].intersection(polelines[0]))

    res_fun_poly = res_fr_poly
    res_fun_poly.translate_start_to_point(intersections[-1])
...
def method_of_joints(ax_method_of_joints,acting_forces,node_number,unknown_forces,system):
    ...
    if unknown_forces == 1:
        ...
        intersection = member_lines[0][0].intersection(member_lines[1][0])
...
def calculate_subsystem_forces(ax_sect_fr_poly, ax_sect_fun_poly,intersected_elements,new_ex_forces_list,new_node_list,system):
    ...
    if len(intersected_elements) == 3: #must be 3 otherwise subsystems arent connected properly (statische bestimmtheit)
        ...
        res_fun_poly.translate_end_to_point(point_of_action)

        # Translate Line to start of R
        line = translate_to_point(line,res_fun_poly.p1)

        intersection = line.intersection(sub_member_lines[0])[0]

        # sub_member_force[0]
        dx = res_fun_poly.p2.x-intersection.x
        dy = res_fun_poly.p2.y-intersection.y
        dx1 = sub_member_lines[0].p2.x-sub_member_lines[0].p1.x

        amount = float(res_fun_poly.p2.distance(intersection)/system.scale)
        ...
        sub_member_forces[0][0].translate_end_to_point(acting_point)

        #Res of sub_member_force[1] and sub_member_force[2]
        dx = intersection.x-res_fun_poly.p1.x
        dy = intersection.y-res_fun_poly.p1.y
        dx1 = line.p2.x-line.p1.x

        amount = float(res_fun_poly.p1.distance(intersection)/system.scale)
        ...
        res_12.translate_end_to_point(acting_point)
    ...
    for i in range(len(forces)):
        if type(forces[i][1]) == type([]): # is member_force
            ...
            if round(float(forces[i][0].amount),5) < 0:
                forces[i][0].mirror_at_startpoint()
                angle = calculate_angle(forces[i][0].p2,node)
                forces[i][0].mirror_at_startpoint() # set back to how it was
            else:
                angle = calculate_angle(forces[i][0].p2,node)
