...
from mechanics_utilities import funicular_polygon
...
class System():
    ...
    def import_from_json(self):
        ...
        for i in range(len(nodes)):
            self.node_list.append([Point2D(nodes[i]['coords'][0],nodes[i]['coords'][1]),
                                   nodes[i]['id']])
        ...
        for i in range(len(elements)):
            self.element_list.append([elements[i]['connect'],elements[i]['id']])
        ...
        for i in range(len(boundary)):
            ...
            ##PMT why are there 2 entries for the boundary list for 1 fixity?

            self.boundary_list.append([boundary[i]['fixity'][0],False,boundary[i]['node_id'],counter+self.n_members+len(self.ex_forces_list)])
            counter +=1
            self.boundary_list.append([False,boundary[i]['fixity'][1],boundary[i]['node_id'],counter+self.n_members+len(self.ex_forces_list)])
            counter +=1
