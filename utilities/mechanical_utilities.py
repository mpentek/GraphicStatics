...
def get_force_diagram(forces):
    ...
    force_diagram['force_base_points'].append(Node2D('p' + force_diagram['forces'][-1].id, [s_coord_x, s_coord_y]))

    e_coord_x = s_coord_x + force_diagram['forces'][-1].magnitude * force_diagram['forces'][-1].direction[0]
    e_coord_y = s_coord_y + force_diagram['forces'][-1].magnitude * force_diagram['forces'][-1].direction[1]
    ...
    for i in range(len(forces)-1):
        ...
        force_diagram['force_base_points'].append(Node2D('p_' + force_diagram['forces'][-1].id, [s_coord_x, s_coord_y]))
    ...
    force_diagram['resultant'] = Force2D('r_ext', force_diagram['force_base_points'][0], force_diagram['force_base_points'][0].coordinates, [resultant_end[0] - resultant_start[0], resultant_end[1] - resultant_start[1]])
...
def get_space_diagram(force_diagram, initial_offset_factor=0.1):
    ...
    init_p_coords_x = force_diagram['forces'][0].coordinates[0] + force_diagram['forces'][0].direction[0] * force_diagram['forces'][0].magnitude * initial_offset_factor
    init_p_coords_y = force_diagram['forces'][0].coordinates[1] + force_diagram['forces'][0].direction[1] * force_diagram['forces'][0].magnitude * initial_offset_factor
    ...
    space_diagram['intersection_points'].append(Node2D(force_diagram['resultant'].id, [cur_p_coords_x, cur_p_coords_y]))

    for i in range(len(space_diagram['intersection_points'])-1):
        space_diagram['pole_segments'].append(Segment2D(force_diagram['pole_segments'][i+1].id,[space_diagram['intersection_points'][i], space_diagram['intersection_points'][i+1]]))

    # add last segment as well
    space_diagram['pole_segments'].append(Segment2D(force_diagram['pole_segments'][0].id,[space_diagram['intersection_points'][0], space_diagram['intersection_points'][-1]]))
...
def get_reactions(acting_line, decomposed_forces, directions, fixity_locations, fixity_type, fixity_node_id):
    ...
    if n_fixed[0] == 1:
        ...
        idx = [i for i in range(len(fixity_type)) if fixity_type[i][0] == True][0]
        ...
        if abs(fixity_locations[0][1]-fixity_locations[1][1]) < TOL:
            ...
            reaction_forces = decompose_force_by_inverse_proportion(acting_line, directions[0], fixity_locations, decomposed_forces[0], fixity_node_id)
    ...
    if n_fixed[1] == 1:
        # should only return one element - [0] needed in the end
        idx = [i for i in range(len(fixity_type)) if fixity_type[i][1] == True][0]
        loc = fixity_locations[idx]
        reactions.append(Force2D(None,
                            fixity_node_id[1],
                            loc,
        ...
        if abs(fixity_locations[0][0]-fixity_locations[1][0]) < TOL:
            ...
            reaction_forces = decompose_force_by_inverse_proportion(acting_line, directions[1], fixity_locations, decomposed_forces[1],fixity_node_id)
